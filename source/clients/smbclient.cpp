...
int SmbClient::Connect(const std::string &url, const std::string &user, const std::string &pass)
{
	...
	if (smb2 == NULL)
	{
		snprintf(response, 1023, "%s", "Failed to init SMB context");
		return 0;
	}
	...
	{
		snprintf(response, 1023, "%s", smb2_get_error(smb2));
		return 0;
	}
...
std::vector<DirEntry> SmbClient::ListDir(const std::string &path)
{
	...
	if (dir == NULL)
	{
		snprintf(status_message, 1023,"%s - %s", lang_strings[STR_FAIL_READ_LOCAL_DIR_MSG], smb2_get_error(smb2));
		return out;
	}
